...
startInducer()
{
	...
	level.players[0] playsound ("statue_activate");
	...
	level.players[0] playsound ("statue_deactivate");
//	foreach ( player in get_players() )
...
roundChecker()
{
	if(getDvarInt("rampage_max_round") == 0)
	{
		return;
	}
...
spawnInducer()
{
	...
	while(1)
	{
		...
		if ((level.round_number < getDvarInt("rampage_max_round") || getDvarInt("rampage_max_round") == 0) && (level.ragestarted == 0) && (level.finishedrampage == 0))
		{
			if ( i usebuttonpressed() )
			{
				if (level.rampagevoting == 0)
				{
					...
					if (level.votingsuccess == 1)
					{
						...
						if (getDvarInt("rampage_max_round") <= 5 && getDvarInt("rampage_max_round") > 0)
						{
...
//						break;
		...
		else if ((level.round_number > getDvarInt("rampage_max_round")) && (level.ragestarted == 0) && (level.finishedrampage == 0) && getDvarInt("rampage_max_round") > 0)
		{
...
{
	if(audio != "" && (getDvarInt("enable_rampage_vox") == 1 || getDvarInt("enable_rampage_vox") == 2))
	{
		self playlocalsound(audio);
	}

	if(getDvarInt("enable_rampage_vox") == 1 || getDvarInt("enable_rampage_vox") == 3)
	{	
